...
export default function ProductDetailPage() {
  ...
  const {
    ...
    isLoading,
    isError,
  } = _productService.useProductDetail(id as string)
  ...
  const { data: reviewsData } = _productService.useProductReviews(
    product?.id || '',
    {
      page: 1,
      limit: 1,
  ...
  if (isError || !product) {
    return (
      <div className='flex flex-col items-center justify-center min-h-screen gap-4'>
        ...
        <Link
          href='/shop'
          className='bg-black text-white px-8 py-3 rounded-full font-bold uppercase tracking-widest text-xs'
        >
          Quay lại cửa hàng
        </Link>
      </div>
    ...
    <div className='bg-white min-h-screen'>
      ...
      <div className='main-container mx-auto px-4 py-6'>
        <div className='flex items-center justify-between'>
          <Link
            href='/shop'
            className='flex items-center gap-2 text-[10px] font-black text-neutral-400 hover:text-black transition-colors uppercase tracking-widest group'
          >
          ...
          <div className='flex items-center gap-2 text-[10px] text-neutral-400 font-semibold uppercase tracking-widest'>
            ...
            <ChevronRight className='w-3 h-3 text-neutral-300' />
            <span className='text-neutral-500 font-bold truncate max-w-[150px] md:max-w-none'>
              {product.name}
            </span>
          </div>
        </div>
        ...
        <div className='mt-16 bg-[#FAF6F0] p-6 flex flex-col md:flex-row items-center justify-between gap-4 border border-[#e8dfd5]/60 rounded-sm'>
          <div className='flex items-center gap-4'>
            ...
            <p className='text-xs md:text-sm text-[#5c4e43] font-semibold leading-relaxed'>
              <strong>10% lợi nhuận</strong> được trích từ mỗi đơn hàng để thực
              hiện các hoạt động thiện nguyện,
              <br className='hidden md:inline' /> hỗ trợ trẻ em và cộng đồng có
              hoàn cảnh khó khăn.
            </p>
          </div>
          <Link
            href='/about'
            className='flex items-center gap-1.5 text-xs font-black uppercase tracking-wider text-black hover:opacity-75 transition-opacity whitespace-nowrap'
          >
      ...
      <section
        ref={tabsRef}
        className='py-16 border-t border-neutral-100 bg-white'
      >
          ...
          <div className='min-h-[300px]'>
            {activeTab === 'description' && (
              <div className='grid grid-cols-1 md:grid-cols-2 gap-12 items-start'>
                ...
                <div className='aspect-[4/3] rounded-sm overflow-hidden sticky top-20 right-0 bg-neutral-100 border border-neutral-100'>
                  <img
                    src={
                ...
                <table className='w-full text-sm text-neutral-600 border-collapse'>
                  <tbody>
                    <tr className='border-b border-neutral-100'>
                      <td className='py-4 font-black text-xs uppercase tracking-wider text-black w-1/3'>
                        Thương hiệu
                      </td>
                      <td className='py-4'>{product.brand?.name || 'LUNÉ'}</td>
                    </tr>
                    <tr className='border-b border-neutral-100'>
                      <td className='py-4 font-black text-xs uppercase tracking-wider text-black'>
                        Danh mục
                      </td>
                      <td className='py-4'>
                    ...
                    <tr className='border-b border-neutral-100'>
                      ...
                      <td className='py-4 font-bold text-black'>
                        {product.slug?.toUpperCase() || product.id}
                      </td>
                    </tr>
                    <tr className='border-b border-neutral-100'>
                      <td className='py-4 font-black text-xs uppercase tracking-wider text-black'>
            ...
            {activeTab === 'care' && (
              <div className='max-w-xl mx-auto space-y-4 text-sm text-neutral-600 font-medium leading-relaxed'>
                <p className='font-bold text-black mb-2'>
                  Hướng dẫn bảo quản trang phục LUNÉ:
                </p>
                <ul className='list-disc list-inside space-y-2'>
                  <li>
                    Giặt tay hoặc giặt máy bằng nước lạnh (không quá 30 độ C)
                    với chu kỳ xả nhẹ nhàng.
                  </li>
                  <li>
                    Sử dụng xà phòng trung tính, tránh dùng chất tẩy trắng mạnh.
                  </li>
                  <li>
                    Không ngâm chung đồ tối màu với đồ sáng màu để tránh loang
                    màu.
                  </li>
                  <li>
                    Phơi khô trong bóng râm, tránh tiếp xúc trực tiếp dưới ánh
                    nắng gay gắt.
                  </li>
                  <li>
                    Ủi (là) sản phẩm ở nhiệt độ thấp hoặc trung bình khi cần
                    thiết.
                  </li>
                </ul>
              </div>
