...
export const _menuApi = {
  ...
  fetchMenuById: async (id: string) => {
    const res = await https.get<ApiResponse<Menu>>(`/navigations/${id}`)
    return res.data
  },

  createMenu: async (data: MenuInput) => {
    const res = await https.post<ApiResponse<Menu>>('/navigations', data)
    return res.data
  },

  updateMenu: async (id: string, data: Partial<MenuInput>) => {
    const res = await https.put<ApiResponse<Menu>>(`/navigations/${id}`, data)
    return res.data
  },

  deleteMenu: async (id: string) => {
    const res = await https.delete<ApiResponse<null>>(`/navigations/${id}`)
    return res.data
  }
}
